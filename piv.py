...
def xcorr_norm(img, kernel):
    ...
    kernel = kernel/scipy.sqrt(scipy.sum(kernel*kernel))
...
def find_flow(img1, img2, ii_all, jj_all,
    ...
    for i_point in range(ii_all.size):
        ...
        kernel = img1[ii-li_half:ii+li_half+1, jj-lj_half:jj+lj_half+1]
        ...
        if di==0 or di==cc_shape[0]-1 or dj==0 or dj==cc_shape[1]-1\
                or numpy.max(cc) < 0.5:
            di_all[i_point] = 0
            dj_all[i_point] = 0
